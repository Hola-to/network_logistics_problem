...
> Generated: 2025-12-09 16:54:58

## Table of Contents

- [web](#web)
- [web/src/api](#web-src-api)
- [web/src/api/services](#web-src-api-services)
- [web/src/hooks](#web-src-hooks)
- [web/src](#web-src)
- [web/src/stores](#web-src-stores)
...
> Path: `web/package-lock.json` | Lines: 5091
...
> Path: `web/package.json` | Lines: 48
...
> Path: `web/postcss.config.js` | Lines: 6
...
> Path: `web/src/api/client.ts` | Lines: 25
...
import { GatewayService } from "@gen/logistics/gateway/v1/gateway_connect";
...
export const client = createPromiseClient(GatewayService, transport);
...
> Path: `web/src/api/interceptors.ts` | Lines: 84
...
> Path: `web/src/api/services/analytics.ts` | Lines: 47
...
export const analyticsService = {
  // Переименовать для согласованности с вызовами
  analyzeGraph: (params: {
    graph: Graph;
  ...
  getBottlenecks: (graph: Graph, utilizationThreshold = 0.9, topN = 10) =>
    client.getBottlenecks({ graph, utilizationThreshold, topN }),

  compareScenarios: (
    baseline: Graph,
    scenarios: Array<{ name: string; graph: Graph }>,
  ) =>
    client.compareScenarios({
      baseline,
      scenarios: scenarios.map((s) => ({ name: s.name, graph: s.graph })),
    }),
};
```
...
> Path: `web/src/api/services/audit.ts` | Lines: 66
  ...
  getLogs: (params?: {
    ...
    client.getAuditLogs({
      startTime: params?.startTime
        ...
        : undefined,
      endTime: params?.endTime ? Timestamp.fromDate(params.endTime) : undefined,
...
> Path: `web/src/api/services/auth.ts` | Lines: 37
...
export const authService = {
   ...
   * Вход в систему
   */
  login: (request: LoginRequest) => client.login(request),

  /**
   * Регистрация
   */
  register: (request: RegisterRequest) => client.register(request),

  /**
   * Обновление токена
   */
  refreshToken: (refreshToken: string) => client.refreshToken({ refreshToken }),
   ...
   * Валидация токена
   */
  validateToken: (token: string) => client.validateToken({ token }),
};
```

#### `history.ts`

> Path: `web/src/api/services/history.ts` | Lines: 57
...
export const historyService = {
  ...
  getCalculation: (calculationId: string) =>
    client.getCalculation({ calculationId }),

  listCalculations: (params?: {
    limit?: number;
    offset?: number;
    algorithm?: Algorithm;
    tags?: string[];
    ...
    client.listCalculations({
      ...
      createdAfter: params?.createdAfter
        ? Timestamp.fromDate(params.createdAfter)
        : undefined,
      createdBefore: params?.createdBefore
        ? Timestamp.fromDate(params.createdBefore)
        : undefined,
  ...
  deleteCalculation: (calculationId: string) =>
    client.deleteCalculation({ calculationId }),
...
> Path: `web/src/api/services/index.ts` | Lines: 7
...
> Path: `web/src/api/services/report.ts` | Lines: 99
...
export const reportService = {
  ...
  generateFlowReport: (
    ...
    client.generateReport({
      ...
      // oneof source - для @bufbuild/protobuf используется объект с полем напрямую
      flowSource: {
        graph,
        result,
        metrics,
   ...
   * Получение отчёта
   */
  get: (reportId: string) => client.getReport({ reportId }),

  /**
   * Скачивание отчёта (streaming) - возвращает AsyncIterable
   */
  download: (reportId: string): AsyncIterable<ReportChunk> => {
    return client.downloadReport({ reportId });
  },

  /**
   * Скачивание отчёта как Blob
   */
  downloadAsBlob: async (reportId: string): Promise<Blob> => {
    const chunks: Uint8Array[] = [];

    // Используем download который возвращает AsyncIterable
    for await (const chunk of client.downloadReport({ reportId })) {
   ...
   * Удаление отчёта
   */
  delete: (reportId: string) => client.deleteReport({ reportId }),

  /**
   * Получение поддерживаемых форматов
   */
  getFormats: () => client.getReportFormats({}),
};
```

#### `simulation.ts`

> Path: `web/src/api/services/simulation.ts` | Lines: 161
...
export const simulationService = {
   ...
   * Возвращает AsyncIterable напрямую
   */
  monteCarloStream: (
    graph: Graph,
    uncertainties: UncertaintySpec[],
    config?: Partial<MonteCarloConfig>,
    algorithm: Algorithm = Algorithm.DINIC,
  ): AsyncIterable<MonteCarloProgressEvent> => {
    return client.runMonteCarloStream({
      graph,
      config: {
        numIterations: 1000,
        confidenceLevel: 0.95,
  ...
  getSimulation: (simulationId: string) =>
    client.getSimulation({ simulationId }),

  listSimulations: (limit = 20, offset = 0, simulationType?: string) =>
    client.listSimulations({
      limit,
      offset,
  ...
  deleteSimulation: (simulationId: string) =>
    client.deleteSimulation({ simulationId }),
};
```

#### `solver.ts`

> Path: `web/src/api/services/solver.ts` | Lines: 76

```typescript
import { client } from "../client";
import { Algorithm } from "@gen/logistics/common/v1/common_pb";
import type { Graph, SolveOptions } from "@gen/logistics/common/v1/common_pb";
import type {
...
export const solverService = {
  ...
  /**
   * Streaming решение - возвращает AsyncIterable напрямую
   * Использование: for await (const event of solverService.solveStream(...)) { }
   */
  solveStream: (params: {
    graph: Graph;
    algorithm?: Algorithm;
    options?: Partial<SolveOptions>;
  }): AsyncIterable<SolveProgressEvent> => {
    // Connect-RPC streaming методы возвращают AsyncIterable напрямую
    return client.solveGraphStream({
      graph: params.graph,
      algorithm: params.algorithm || Algorithm.DINIC,
      options: {
        returnPaths: true,
  ...
  getAlgorithms: () => client.getAlgorithms({}),
  health: () => client.health({}),
  readiness: () => client.readinessCheck({}),
  info: () => client.info({}),
};
```
...
> Path: `web/src/hooks/useAudit.ts` | Lines: 52
...
> Path: `web/src/hooks/useAuth.ts` | Lines: 95
...
> Path: `web/src/hooks/useSolver.ts` | Lines: 138

```typescript
import { useMutation, useQuery } from "@tanstack/react-query";
import toast from "react-hot-toast";
import { solverService, historyService } from "@/api/services";
import { useGraphStore, Algorithm } from "@/stores/graphStore";
import type { Graph } from "@gen/logistics/common/v1/common_pb";
...
export function useSolver() {
  ...
  const solveMutation = useMutation({
    mutationFn: async () => {
      ...
      return solverService.solve(graph, store.algorithm);
    },
    onMutate: () => {
  ...
  // Calculate logistics (full pipeline)
...
> Path: `web/src/index.css` | Lines: 132
...
@layer components {
    ...
    .btn-ghost {
        @apply btn bg-transparent text-gray-600 shadow-none
           hover:bg-gray-100;
    }
...
> Path: `web/src/stores/authStore.ts` | Lines: 66
...
> Path: `web/src/stores/graphStore.ts` | Lines: 252
...
interface GraphState {
  ...
  updateNode: (id: bigint, updates: Partial<Node>) => void;
  removeNode: (id: bigint) => void;

  // Actions - Edges
  addEdge: (edge: Partial<Edge>) => Edge | null;
  updateEdge: (from: bigint, to: bigint, updates: Partial<Edge>) => void;
  removeEdge: (from: bigint, to: bigint) => void;

  // Actions - Graph
  setSourceSink: (sourceId: bigint | null, sinkId: bigint | null) => void;
  ...
  // Actions - UI
  selectNode: (id: bigint | null) => void;
  selectEdge: (key: { from: bigint; to: bigint } | null) => void;
...
export const useGraphStore = create<GraphState>((set, get) => ({
  ...
  // Node actions
  addNode: (nodeData) => {
    const id = nodeData.id ?? nodeIdCounter++;
    const node: Node = {
      id,
      x: nodeData.x ?? 0,
      y: nodeData.y ?? 0,
      type: nodeData.type ?? NodeType.INTERSECTION,
      name: nodeData.name ?? `Узел ${id}`,
      metadata: nodeData.metadata ?? {},
      supply: nodeData.supply ?? 0,
  ...
  removeNode: (id) =>
    set((state) => ({
      ...
      sourceId: state.sourceId === id ? null : state.sourceId,
      sinkId: state.sinkId === id ? null : state.sinkId,
  ...
  addEdge: (edgeData) => {
    ...
    const edge: Edge = {
      from,
      to,
      capacity: edgeData.capacity ?? 10,
      cost: edgeData.cost ?? 1,
      length: edgeData.length ?? 1,
      roadType: edgeData.roadType ?? RoadType.PRIMARY,
      currentFlow: 0,
  ...
  removeEdge: (from, to) =>
    set((state) => ({
      ...
      selectedEdgeKey:
        state.selectedEdgeKey?.from === from && state.selectedEdgeKey?.to === to
          ? null
  ...
  // Graph actions
  setSourceSink: (sourceId, sinkId) => set({ sourceId, sinkId }),
...
> Path: `web/src/utils/formatters.ts` | Lines: 37
...
> Path: `web/src/vite-env.d.ts` | Lines: 11
...
> Path: `web/tailwind.config.js` | Lines: 24
...
> Path: `web/tsconfig.json` | Lines: 26
...
> Path: `web/tsconfig.node.json` | Lines: 10
...
> Path: `web/vite.config.ts` | Lines: 22
